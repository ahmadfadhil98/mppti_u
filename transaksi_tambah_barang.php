...
if (isset($_POST['delete'])) {
    ...
    $data = $conn->query("SELECT count(id_transaksi) as id FROM detail_transaksi where id_transaksi='$id_transaksi'");
    while ($row1 = $data->fetch_assoc()) {
        $brag = $row1['id'];
    }

    if ($brag == 0) {
        $pesan_gagal = "Data barang transaksi belum ditambahkan!";
    } else {
